...
REQUEST_STATUS server_status::request_status{REQUEST_COMPLETED_SUCCESSFULY};

auto server_status::get_status() -> int {
    return data_base_status | (JWT_status << DATA_BASE_SHIFT) | (file_parse_status << (DATA_BASE_SHIFT + JWT_SHIFT)) | (request_status << (DATA_BASE_SHIFT + JWT_SHIFT + FILE_PARSER_SHIFT));
}